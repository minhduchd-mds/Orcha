...
if [[ "$health" != *'7.4.0'* || "$health" != *'"product":"Orcha"'* || "$health" != *'hybrid_data'* ]]; then
...
for _ in $(seq 1 60); do
  health="$(curl -fsS "${URL}health" 2>/dev/null || true)"
  [[ "$health" == *'7.4.0'* && "$health" == *'Orcha'* && "$health" == *'hybrid_data'* ]] && break
  sleep 0.25
done
if [[ "$health" != *'7.4.0'* ]]; then osascript -e 'display alert "Orcha" message "Không khởi động được runtime v7.4. Kiểm tra Python/Ollama và log trong ~/Library/Logs/Orcha.log." as critical' >/dev/null 2>&1 || true; exit 1; fi
