...
if ! command -v python3 >/dev/null 2>&1; then osascript -e 'display alert "Orcha" message "Chưa có Python 3. Hãy cài Python 3.10+ rồi mở lại ứng dụng." as critical' >/dev/null 2>&1 || true; open "https://www.python.org/downloads/macos/" || true; exit 1; fi
...
if [[ "$health" != *'7.5.0'* || "$health" != *'"product":"Orcha"'* || "$health" != *'ui_foundation'* ]]; then
...
for _ in $(seq 1 60); do
  health="$(curl -fsS "${URL}health" 2>/dev/null || true)"
  [[ "$health" == *'7.5.0'* && "$health" == *'Orcha'* && "$health" == *'ui_foundation'* ]] && break
  sleep 0.25
done
if [[ "$health" != *'7.5.0'* ]]; then osascript -e 'display alert "Orcha" message "Không khởi động được runtime v7.5. Kiểm tra Python/Ollama và log trong ~/Library/Logs/Orcha.log." as critical' >/dev/null 2>&1 || true; exit 1; fi
