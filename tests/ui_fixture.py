...
def main():
    with tempfile.TemporaryDirectory(prefix='orcha-ui-') as directory:
        ...
        class Ollama(BaseHTTPRequestHandler):
            ...
            def do_GET(self):self.respond({'models':[{'name':'qwen3:0.6b-q4_K_M'},{'name':'kimik3-lite-v3:latest'},{'name':'moondream:1.8b-v2-q2_K'}]})
            def do_POST(self):
        ...
        server=ThreadingHTTPServer(('127.0.0.1',11437),app.H70);server.ollama=f'http://127.0.0.1:{fake.server_port}';server.profile='balanced';server.model='kimik3-lite-v3';server.model_mode='auto'
