...
if [[ "$health" != *'"version": "6.9.0"'* && "$health" != *'"version":"6.9.0"'* ]]; then
...
if [[ "$health" != *'6.9.0'* ]]; then osascript -e 'display alert "KimiK3-Lite Studio" message "Không khởi động được runtime v6.9. Kiểm tra Python/Ollama và log trong ~/Library/Logs." as critical' >/dev/null 2>&1 || true; exit 1; fi
