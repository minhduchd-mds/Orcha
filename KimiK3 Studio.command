...
if [[ "$health" != *'"version": "7.1.0"'* && "$health" != *'"version":"7.1.0"'* ]]; then
...
if [[ "$health" != *'7.1.0'* ]]; then osascript -e 'display alert "KimiK3-Lite Studio" message "Không khởi động được runtime v7.1. Kiểm tra Python/Ollama và log trong ~/Library/Logs." as critical' >/dev/null 2>&1 || true; exit 1; fi
