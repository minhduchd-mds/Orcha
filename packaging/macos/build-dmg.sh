...
if [[ "$health" != *'7.2.0'* || "$health" != *'autonomous_project_planner'* ]]; then
...
for _ in $(seq 1 60); do
  health="$(curl -fsS "${URL}health" 2>/dev/null || true)"
  [[ "$health" == *'7.2.0'* && "$health" == *'autonomous_project_planner'* ]] && break
  sleep 0.25
done
if [[ "$health" != *'7.2.0'* ]]; then osascript -e 'display alert "KimiK3-Lite Studio" message "Không khởi động được runtime v7.2." as critical' || true; exit 1; fi
...
<key>CFBundleVersion</key><string>7.2.0</string>
<key>CFBundleShortVersionString</key><string>7.2.0</string>
