...
if ! curl -fsS "${URL}health" >/dev/null 2>&1; then nohup python3 "$ROOT/app/studio_server.py" --host 127.0.0.1 --port "$PORT" --profile balanced >"$HOME/Library/Logs/KimiK3-Lite-Studio.log" 2>&1 & fi
