...
if [[ "$health" != *'7.4.0'* || "$health" != *'Orcha'* || "$health" != *'hybrid_data'* ]]; then
...
if [[ "$health" != *'7.4.0'* ]]; then osascript -e 'display alert "Orcha" message "Không khởi động được runtime v7.4." as critical' || true; exit 1; fi
...
<key>CFBundleVersion</key><string>7.4.0</string>
<key>CFBundleShortVersionString</key><string>7.4.0</string>
