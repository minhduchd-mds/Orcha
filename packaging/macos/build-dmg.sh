...
if [[ "$health" != *'7.1.0'* ]]; then
  curl -fsS -X POST -H 'Content-Type: application/json' -d '{}' "${URL}api/app/shutdown" >/dev/null 2>&1 || true
  sleep 0.5
  nohup python3 "$ROOT/app/studio_server_v71.py" --host 127.0.0.1 --port "$PORT" --profile balanced >"$HOME/Library/Logs/KimiK3-Lite-Studio.log" 2>&1 &
fi
for _ in $(seq 1 60); do
  health="$(curl -fsS "${URL}health" 2>/dev/null || true)"
  [[ "$health" == *'7.1.0'* && "$health" == *'durable_agent_team'* ]] && break
  sleep 0.25
done
if [[ "$health" != *'7.1.0'* ]]; then osascript -e 'display alert "KimiK3-Lite Studio" message "Không khởi động được runtime v7.1." as critical' || true; exit 1; fi
...
<key>CFBundleVersion</key><string>7.1.0</string>
...
hdiutil create -volname "KimiK3 Lite Studio" -srcfolder "$DMGROOT" -ov -format UDZO "$OUT/KimiK3-Lite-v7.1-macOS.dmg"
