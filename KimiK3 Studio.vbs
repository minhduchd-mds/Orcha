...
Function ServerReady()
  ...
  r.Send
  body = r.ResponseText
  ServerReady = (Err.Number = 0 And r.Status = 200 And InStr(1, body, "7.1.0", 1) > 0 And InStr(1, body, "durable_agent_team", 1) > 0)
  Err.Clear
  On Error GoTo 0
End Function
Sub StopOldServer()
...
If Not ServerReady() Then
  StopOldServer
  cmd = "cmd.exe /d /s /c """"" & root & "\scripts\_python.cmd"" """ & root & "\app\studio_server_v71.py"" --profile balanced --port 11435"""
  sh.Run cmd, 0, False
  ok = False
  For i = 1 To 60
    WScript.Sleep 250
...
End If
If Not ok Then
  MsgBox "KimiK3-Lite Studio v7.1 khong khoi dong duoc. Hay chay INSTALL.bat hoac kiem tra Python/Ollama.", 16, "KimiK3-Lite"
  WScript.Quit 1
End If
