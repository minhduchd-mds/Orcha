...
srv=Server('orcha-autocad','1.2.0')
def _require_windows():
    if os.name!='nt':raise RuntimeError('AutoCAD MCP bridge currently supports Windows only')
def _ps(body,args=None,timeout=30):
    _require_windows();payload=base64.b64encode(json.dumps(args or {},ensure_ascii=False).encode()).decode();env=os.environ.copy();env['ORCHA_AUTOCAD_ARGS_B64']=payload;prefix=r'''$ErrorActionPreference='Stop';$raw=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($env:ORCHA_AUTOCAD_ARGS_B64));$a=$raw|ConvertFrom-Json;$app=[Runtime.InteropServices.Marshal]::GetActiveObject('AutoCAD.Application');$doc=$app.ActiveDocument;''';cp=subprocess.run(['powershell.exe','-NoProfile','-NonInteractive','-Command',prefix+body],capture_output=True,text=True,encoding='utf-8',errors='replace',env=env,timeout=timeout)
...
def entity_create(args):
    ...
    script=r'''$type=[string]$a.type;$obj=$null;if($type -eq 'line'){$s=[double[]]@($a.start[0],$a.start[1],$a.start[2]);$e=[double[]]@($a.end[0],$a.end[1],$a.end[2]);$obj=$doc.ModelSpace.AddLine($s,$e)}elseif($type -eq 'polyline'){$flat=New-Object System.Collections.Generic.List[double];foreach($p in $a.points){$flat.Add([double]$p[0]);$flat.Add([double]$p[1])};$obj=$doc.ModelSpace.AddLightWeightPolyline([double[]]$flat.ToArray());$obj.Closed=[bool]$a.closed}elseif($type -eq 'text'){$p=[double[]]@($a.position[0],$a.position[1],$a.position[2]);$obj=$doc.ModelSpace.AddText([string]$a.text,$p,[double]$a.height)}elseif($type -eq 'circle'){$p=[double[]]@($a.center[0],$a.center[1],$a.center[2]);$obj=$doc.ModelSpace.AddCircle($p,[double]$a.radius)};if([string]$a.layer){try{$null=$doc.Layers.Item([string]$a.layer)}catch{$null=$doc.Layers.Add([string]$a.layer)};$obj.Layer=[string]$a.layer};$obj.Update();@{created=$true;type=$type;handle=$obj.Handle;layer=$obj.Layer;document=$doc.FullName}|ConvertTo-Json -Compress''';return _ps(script,payload)
...
def dimension_create(args):
    p={'start':_pt(args.get('start')),'end':_pt(args.get('end')),'location':_pt(args.get('location')),'layer':str(args.get('layer') or '')[:120]};script=r'''$s=[double[]]@($a.start[0],$a.start[1],$a.start[2]);$e=[double[]]@($a.end[0],$a.end[1],$a.end[2]);$l=[double[]]@($a.location[0],$a.location[1],$a.location[2]);$o=$doc.ModelSpace.AddDimAligned($s,$e,$l);if([string]$a.layer){try{$null=$doc.Layers.Item([string]$a.layer)}catch{$null=$doc.Layers.Add([string]$a.layer)};$o.Layer=[string]$a.layer};$o.Update();@{created=$true;type='dimension';handle=$o.Handle;layer=$o.Layer;document=$doc.FullName}|ConvertTo-Json -Compress''';return _ps(script,p)
@srv.tool('autocad.entity.delete','Delete one entity by stable AutoCAD handle in the same drawing.',{'type':'object','required':['handle','document'],'properties':{'handle':{'type':'string'},'document':{'type':'string'}}})
def entity_delete(args):
    h=str(args.get('handle') or '').strip();document=str(args.get('document') or '').strip()
    if not h or len(h)>32:raise RuntimeError('Handle không hợp lệ')
    if not document:raise RuntimeError('Drawing identity required for delete/rollback')
    return _ps("if($doc.FullName -ne [string]$a.document){throw 'Active drawing changed'};$h=[string]$a.handle;$o=$doc.HandleToObject($h);$o.Delete();@{deleted=$true;handle=$h}|ConvertTo-Json -Compress",{'handle':h,'document':document})
