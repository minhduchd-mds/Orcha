...
class H70(v69.H69):
    ...
    def do_POST(self):
        try:
            b=self.body()
            with core.project_scope(b.get('project_id')):return self._post70()
        except (ValueError,TypeError) as exc:return self.send_json(400,{'error':str(exc)})
    ...
    def _post70(self):
        ...
        if path=='/api/chat':
            try:
                b=self.body()
                with core.project_scope(b.get('project_id')):return self._chat69(b)
            except (ValueError,TypeError) as exc:return self.send_json(400,{'error':str(exc)})
        ...
        if path=='/api/data/sync':
            try:
                ...
                return self.send_json(200,data_sync.sync_source(sid) if sid else {'results':data_sync.sync_due()})
            except Exception as e:return self.send_json(409,{'error':str(e)})
        ...
        if path=='/api/projects' or path.startswith('/api/projects/'):
            ...
            try:
                ...
                if len(parts)==6 and parts[3]=='tasks':
                    if parts[5]=='execute':return self.send_json(200,supervisor.execute(pid,parts[4],explicit=True,profile=self.server.profile,host=self.server.ollama))
                    if parts[5]=='continue':return self.send_json(200,supervisor.continue_task(pid,parts[4],str(b.get('scope') or 'once'),b.get('approved') is True))
                    if parts[5]=='retry':return self.send_json(200,projects.retry_task(pid,parts[4]))
                if len(parts)==5 and parts[3]=='tasks':
                    if any(k not in {'title','description'} for k in b):raise ValueError('Task state is managed by the executor; only title and description are editable')
                    return self.send_json(200,projects.update_task(pid,parts[4],b))
                ...
                if len(parts)==5 and parts[3]=='supervisor':
                    ...
                    opts={'profile':str(b.get('profile') or self.server.profile),'model':b.get('model'),'host':self.server.ollama,'session_id':str(b.get('session_id') or 'project-supervisor')}
...
def main():
    ...
    ap=argparse.ArgumentParser();ap.add_argument('--host',default='127.0.0.1');ap.add_argument('--port',type=int,default=11435)
    ap.add_argument('--ollama',default='http://127.0.0.1:11434');ap.add_argument('--profile',default='balanced');ap.add_argument('--model');a=ap.parse_args()
    if a.host not in {'127.0.0.1','localhost'}:ap.error('Desktop API must bind to loopback')
    legacy=v69._legacy_v64().legacy
    cfg=legacy.profiles().get(a.profile)
    if not isinstance(cfg,dict):ap.error('Unknown profile')
    with storage.runtime_lease():
        srv=ThreadingHTTPServer((a.host,a.port),H70)
        scheduler=None
        try:
            legacy.workflows.ensure();recovered=harness.recover_incomplete();skill_builder.migrate_user_skills()
            projects.recover_incomplete();legacy.workflows.recover_incomplete();legacy.start_ollama(a.ollama)
            scheduler=data_sync.Scheduler().start()
            srv.ollama=a.ollama;srv.profile=a.profile;srv.model=a.model or cfg['ollama_name'];srv.model_mode='auto'
            print(f'Orcha v7.6 http://{a.host}:{a.port} · recovered={len(recovered)}',flush=True)
            srv.serve_forever()
        finally:
            if scheduler:scheduler.stop()
            srv.server_close();mcp_transport.close_all()
