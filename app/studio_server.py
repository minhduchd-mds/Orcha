...
def profiles():
    ...
    except Exception: return {'balanced':{'base':'qwen3:0.6b-q4_K_M','ollama_name':'kimik3-lite-v3','published_model_size':'~522 MB'}}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=='/health': return self.send_json(200,{'ok':True,'version':'5.1.0'})
        if path=='/api/studio/status':
            ms=ollama_models(self.server.ollama); ps=profiles(); return self.send_json(200,{'ollama':bool(ms),'models':ms,'profile':self.server.profile,'model':self.server.model,'model_ready':any(x.split(':')[0]==self.server.model.split(':')[0] for x in ms),'profiles':ps,'index_chunks':len(core.load_index()),'memory_items':len(core.load_memory())})
    ...
    def do_POST(self):
        ...
        if path=='/api/setup/use-profile':
            p=str(b.get('profile','balanced')); cfg=profiles().get(p) or profiles()['balanced']; self.server.profile=p; self.server.model=cfg['ollama_name']; return self.send_json(200,{'ok':True,'profile':p,'model':self.server.model})
        if path=='/api/folder/select': return self.send_json(200,{'path':choose_folder()})
        if path=='/api/index':
            try: f,c=core.index_target(str(b.get('path','')),bool(b.get('reset',False))); return self.send_json(200,{'ok':True,'files':f,'chunks':c})
        ...
        if path in {'/api/chat','/v1/chat/completions'}:
            ...
            try: r=core.answer_query(user,self.server.profile,self.server.model,self.server.ollama,str(b.get('mode','auto')),int(b.get('top_k',6)),int(b.get('timeout',300)))
