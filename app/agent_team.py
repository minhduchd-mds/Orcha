...
def send_message(team_id,target,payload,message_id=None):
    ...
    return {'message':row,'delivery':'steer-boundary' if state=='running' else ('next-turn' if state in {'pending',None} else 'cold-resume'),'target_status':state}

def pending_messages(team_id,target):return MAILBOX.replay(team_id,target)

def ack_messages(team_id,target):
    rows=MAILBOX.replay(team_id,target)
    for row in rows:MAILBOX.ack(team_id,target,row['id'])
    return len(rows)
    ...
    while pending:
        ...
        with ThreadPoolExecutor(max_workers=max(1,min(max_workers,len(batch)))) as ex:
            ...
            for f in as_completed(fut):
                ...
                if r.get('ok'):run['shared_memory'][n['id']]={'output':r.get('output','')[:12000]};ack_messages(rid,n['id'])
...
def self_test():
    ...
    rid='team-selftest-'+uuid.uuid4().hex[:8];a=send_message(rid,'critic',{'text':'first'},'m1');b=send_message(rid,'critic',{'text':'second'},'m2');assert a['message']['seq']==0 and b['message']['seq']==1 and [x['id'] for x in pending_messages(rid,'critic')]==['m1','m2'];ack_messages(rid,'critic');assert pending_messages(rid,'critic')==[]
    print('PASS: agent team graph + durable mailbox')
